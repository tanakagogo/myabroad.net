...
<!-- 削除された過去のぴっクエスト表示 -->
	<div class="text_center"><p>過去のpicquest一覧</p></div>
	<div id="all_picquests">


		<?php for($i=0; $i<count($picquest_rs2); $i++): ?>
			<div class="picquests">
				<a class="picquest_word" href="./each_picquest.php?id=<?= h($picquest_rs2[$i]['id']); ?>"><?= h($picquest_rs2[$i]['picquest']); ?></a>
				<div class="pic_num"><?= h($resultSet5[$i]); ?>pic</div>
			</div>
		<?php endfor; ?>


		<?php if($fake2 != 0): ?>
			<?php for($i=0; $i<$fake2; $i++): ?>
				<div class="fake_picquests">
					<a class="picquest_word"></a>
				</div>
			<?php endfor; ?>
