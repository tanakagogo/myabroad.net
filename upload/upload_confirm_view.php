...
<div class="inner">
	...
	<form action="upload_complete.php" method="post">
		<div class="form005">
			...
			<? endif; ?>
			<p>下のアップロード画像やコメントに問題なければ「いざっ、ぴっクエストを投稿」へ。</p>
			<br>
			<p>ぴっクエストID：<?= $picquest_id; ?></p>
			<p>ぴっクエスト：<?= $picquest; ?></p>
			<p>ファイル名：<?= $name; ?></p>
			<?php if(!$error_message): ?>
				<input type="text" name="picquest" value="<?= $comment; ?>" size="35">
				<br>
			<?php endif; ?>
			<!-- 画像表示 -->
			<?php if($link_path): ?>
				<img src="<?= $link_path; ?>" alt="<?= $comment; ?>">
			<?php endif; ?>
		</div>
		<?php if(!$error_message): ?>
			<!-- <div class="form006"> -->
				<p id="text_center2">いざっ、ぴっクエストを投稿</p>
<!-- 				<br> -->
				<input class="btn_design2" type="submit" name="btn1" value="行け！ぴっクエスト">
				<input type="hidden" name="action" value="pq_insert_confirm">
	...
	<form id="f" action="../picquest/each_picquest.php" method="post">
			<p id="text_center2">もどって、ぴっクエストを書きなおししまする。</p>
